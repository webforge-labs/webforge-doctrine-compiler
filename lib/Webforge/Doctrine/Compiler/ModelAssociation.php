...
class ModelAssociation
{
            ...
            if ($this->referencedProperty->getName() === $this->property->getName()) {
                ...
                if ($property->isEntityCollection()) {
                    if ($referencedProperty->isEntityCollection()) {
                        $this->type = 'ManyToMany';
                        $this->owning = isset($property->getDefinition()->isOwning) && $property->getDefinition(
                        )->isOwning; // this might produce a conflict that no side isOwning (check chis later)
                    } else {
                        $this->type = 'OneToMany';
                        $this->owning = false;
                    }
        ...
        if ($this->isUnidirectional()) {
            ...
            if ($this->owning) {
                return sprintf(
                    ...
                    $this->property->getName(),
                    $this->referencedEntity->getName(),
                    $this->referencedProperty->getName()
                );
            } else {
                return sprintf(
                    $format,
    ...
    {
        return $this->isSelfReferencing() && isset($this->referencedProperty) && $this->referencedProperty->getName(
        ) === $this->property->getName();
    }
